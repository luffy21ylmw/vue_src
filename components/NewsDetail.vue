<template>
  <div>
    <div class="d1">
      ...
      <span>{{source}}</span>
      <div class="d3">
        <button v-on:click="agree">点赞|{{agree_num}}</button>
        <button v-on:click="collect">收藏|{{collect_num}}</button>
      </div>
      <div>
    ...
    methods: {
      ...
      initCourseDetail() {
        ...
        this.$axios.request({
          ...
          that.data = response.data.ser;
          that.agree_num = response.data.ser.agree_num;
          that.collect_num = response.data.ser.collect_num;
          that.source = response.data.ser.source.name;
          that.comment_data = response.data.com
        })
      },
      comment_start() {
        var that = this;
...
//            child_id: '',
      ...
      agree() {
        var that = this;
        var url = 'http://127.0.0.1:8000/agree/';
        //console.log(that.$store.state.token);
        this.$axios.request({
          url: url,
          method: 'POST',
